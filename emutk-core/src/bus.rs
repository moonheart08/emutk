...
pub trait Bus<Err> {
    ...
    fn read_tagged<T: ByteRepr>(&mut self, addr: usize, tag: Self::Tag) -> Result<T, Err> {
        self.read(addr)
    }
    /// Write a ByteRepr implementing value to the bus at the selected address, with a tag.
    fn write_tagged<T: ByteRepr>(&mut self, addr: usize, data: T, tag: Self::Tag) -> Result<(), Err> {
...
/// Very simple bus with 32KiB of RAM and 32KiB of ROM, ment for testing.
