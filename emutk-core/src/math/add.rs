...
trait FlaggedAdd<T: OCZSFlagData, Rhs = Self> {
    type Output;

    #[must_use]
    fn flagged_add(self, other: Rhs, flags: T) -> (Self::Output, T);
}
...
impl<T: OCZSFlagData> FlaggedAdd<T> for u8 {
    ...
    fn flagged_add(self, other: Self, mut flags: T) -> (Self::Output, T) {
        ...
        let (_, overflow) = (self as i8).overflowing_add(other as i8);
        let half_carry = (((self & 0xf).wrapping_add(other & 0xf)) & 0x10) == 0x10; // half carry magic
        let half_overflow = (((self & 0x7).wrapping_add(other & 0x7)) & 0x8) == 0x8; // half overflow magic
...
impl<T: OCZSFlagData> FlaggedAdd<T> for u16 {
    ...
    fn flagged_add(self, other: Self, flags: T) -> (Self::Output, T) {
        let (val, carry) = self.overflowing_add(other);
        let (_, overflow) = (self as i16).overflowing_add(other as i16);
        let (_, mut flags) = (self as u8).flagged_add(other as u8, flags);
...
impl<T: OCZSFlagData> FlaggedAdd<T> for u32 {
    ...
    fn flagged_add(self, other: Self, flags: T) -> (Self::Output, T) {
        let (val, carry) = self.overflowing_add(other);
        let (_, overflow) = (self as i32).overflowing_add(other as i32);
        let (_, mut flags) = (self as u16).flagged_add(other as u16, flags);
...
impl<T: OCZSFlagData> FlaggedAdd<T> for u64 {
    ...
    fn flagged_add(self, other: Self, flags: T) -> (Self::Output, T) {
        let (val, carry) = self.overflowing_add(other);
        let (_, overflow) = (self as i64).overflowing_add(other as i64);
        let (_, mut flags) = (self as u32).flagged_add(other as u32, flags);
...
impl<T: OCZSFlagData> FlaggedAdd<T> for u128 {
    ...
    fn flagged_add(self, other: Self, flags: T) -> (Self::Output, T) {
        let (val, carry) = self.overflowing_add(other);
        let (_, overflow) = (self as i128).overflowing_add(other as i128);
        let (_, mut flags) = (self as u64).flagged_add(other as u64, flags);
        flags.set_carry_u64(carry);
        flags.set_overflow_u64(overflow);
